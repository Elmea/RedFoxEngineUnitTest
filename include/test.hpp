#pragma once

#define TEST_START \
LARGE_INTEGER start,end,frequency; \
QueryPerformanceCounter(&start); \
QueryPerformanceFrequency(&frequency); \

#define TEST_STOP \
QueryPerformanceCounter(&end); \
long long cycles = end.QuadPart-start.QuadPart; \
double elapsedTime = (double)(cycles)/frequency.QuadPart; \
elapsedTime *= 1000.f; \

#define TEST_OUT(pExpectedFormat, ...) \
char buffer[256]; \
memset(buffer, 0, sizeof(buffer)); \
std::string output(__FUNCTION__); \
if (result == pExpected) { \
output += " passed in %lf ms (%lli cycle%s)\n"; \
snprintf(buffer, 256, output.c_str(), elapsedTime, cycles, (cycles > 1 ? "s" : "")); \
OutputDebugStringA(buffer); \
} \
else { \
output += std::string(" (") + __FILE__ + " at line " + std::to_string(__LINE__) + \
") failed (expected " + pExpectedFormat + ")\n"; \
snprintf(buffer, 256, output.c_str(), __VA_ARGS__); \
OutputDebugStringA(buffer); \
} \
memset(buffer, 0, sizeof(buffer)); \

#pragma region Float2Test

void Float2CrossProductTest
(const Float2& pA, const Float2& pB, float pExpected) {
    TEST_START;
    float result = pA.CrossProduct(pB);
    TEST_STOP;
    TEST_OUT("%f", pExpected);
}

void Float2DotProductTest
(const Float2& pA, const Float2& pB, float pExpected) {
    TEST_START;
    float result = pA.DotProduct(pB);
    TEST_STOP;
    TEST_OUT("%f", pExpected);
}

void Float2PlanRotationTest
(const Float2& pA, float pAngle, const Float2& pExpected) {
    TEST_START;
    Float2 result = pA;
    result.PlanRotation(pAngle);
    TEST_STOP;
    TEST_OUT("{%f; %f} got {%f; %f}", pExpected.x, pExpected.y, result.x, result.y);
}

void Float2MagnitudeTest(Float2& pA, float pExpected) {
    TEST_START;
    float result = pA.Magnitude();
    TEST_STOP;
    TEST_OUT("%f", pExpected);
}

void Float2NormalizeTest
(Float2& pA, const Float2& pExpected) {
    TEST_START;
    Float2 result = pA.GetNormalized();
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

void Float2LerpTest
(const Float2& pFrom, const Float2& pTo, float pTime, const Float2& pExpected) {
    TEST_START;
    Float2 result = Float2::Lerp(pFrom, pTo, pTime);
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

void Float2AddTest
(const Float2& pA, const Float2& pB, const Float2& pExpected) {
    TEST_START;
    Float2 result = pA + pB;
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

void Float2SubTest
(const Float2& pA, const Float2& pB, const Float2& pExpected) {
    TEST_START;
    Float2 result = pA - pB;
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

void Float2MulSTest
(const Float2& pA, float pB, const Float2& pExpected) {
    TEST_START;
    Float2 result = pA * pB;
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

void Float2DivSTest
(const Float2& pA, float pB, const Float2& pExpected) {
    TEST_START;
    Float2 result = pA / pB;
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

#pragma endregion

#pragma region Float3Test

void Float3CrossProductTest
(const Float3& pA, const Float3& pB, const Float3& pExpected) {
    TEST_START;
    Float3 result = pA.CrossProduct(pB);
    TEST_STOP;
    TEST_OUT("%f %f %f", pExpected.x, pExpected.y, pExpected.z);
}

void Float3DotProductTest
(const Float3& pA, const Float3& pB, float pExpected) {
    TEST_START;
    float result = pA.DotProduct(pB);
    TEST_STOP;
    TEST_OUT("%f", pExpected);
}

void Float3MagnitudeTest(Float3& pA, float pExpected) {
    TEST_START;
    float result = pA.Magnitude();
    TEST_STOP;
    TEST_OUT("%f", pExpected);
}

void Float3NormalizeTest
(Float3& pA, const Float3& pExpected) {
    TEST_START;
    Float3 result = pA.GetNormalized();
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

void Float3LerpTest
(const Float3& pFrom, const Float3& pTo, float pTime, const Float3& pExpected) {
    TEST_START;
    Float3 result = Float3::Lerp(pFrom, pTo, pTime);
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

void Float3AddTest
(const Float3& pA, const Float3& pB, const Float3& pExpected) {
    TEST_START;
    Float3 result = pA + pB;
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

void Float3SubTest
(const Float3& pA, const Float3& pB, const Float3& pExpected) {
    TEST_START;
    Float3 result = pA - pB;
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

void Float3MulSTest
(const Float3& pA, float pB, const Float3& pExpected) {
    TEST_START;
    Float3 result = pA * pB;
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

void Float3DivSTest
(const Float3& pA, float pB, const Float3& pExpected) {
    TEST_START;
    Float3 result = pA / pB;
    TEST_STOP;
    TEST_OUT("%f %f", pExpected.x, pExpected.y);
}

#pragma endregion

#pragma region Float4Test

void Float4DotProductTest
(const Float4& pA, const Float4& pB, float pExpected) {
    TEST_START;
    float result = pA.DotProduct(pB);
    TEST_STOP;
    TEST_OUT("%f", pExpected);
}

void Float4MagnitudeTest(Float4& pA, float pExpected) {
    TEST_START;
    float result = pA.Magnitude();
    TEST_STOP;
    TEST_OUT("%f", pExpected);
}

void Float4NormalizeTest
(Float4& pA, const Float4& pExpected) {
    TEST_START;
    Float4 result = pA.GetNormalized();
    TEST_STOP;
    TEST_OUT("%f %f %f %f", pExpected.x, pExpected.y, pExpected.z, pExpected.w);
}

void Float4AddTest
(const Float4& pA, const Float4& pB, const Float4& pExpected) {
    TEST_START;
    Float4 result = pA + pB;
    TEST_STOP;
    TEST_OUT("%f %f %f %f", pExpected.x, pExpected.y, pExpected.z, pExpected.w);
}

void Float4SubTest
(const Float4& pA, const Float4& pB, const Float4& pExpected) {
    TEST_START;
    Float4 result = pA - pB;
    TEST_STOP;
    TEST_OUT("%f %f %f %f", pExpected.x, pExpected.y, pExpected.z, pExpected.w);
}

void Float4MulSTest
(const Float4& pA, float pB, const Float4& pExpected) {
    TEST_START;
    Float4 result = pA * pB;
    TEST_STOP;
    TEST_OUT("%f %f %f %f", pExpected.x, pExpected.y, pExpected.z, pExpected.w);
}

void Float4DivSTest
(const Float4& pA, float pB, const Float4& pExpected) {
    TEST_START;
    Float4 result = pA / pB;
    TEST_STOP;
    TEST_OUT("%f %f %f %f", pExpected.x, pExpected.y, pExpected.z, pExpected.w);
}

#pragma endregion